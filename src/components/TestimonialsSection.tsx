...
export default function TestimonialsSection() {
  ...
  return (
    <section id="reviews" className="py-14 sm:py-20 bg-slate-950 border-b border-slate-800 text-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-14">
          <div className="text-xs font-bold uppercase tracking-wider text-teal-400 mb-2">Verified Travelers</div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white">Moments That Stay Forever</h2>
          <p className="mt-2 sm:mt-3 text-slate-400 text-xs sm:text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8">
          {reviews.map((r, idx) => (
            <div
              key={idx}
              className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 sm:p-7 flex flex-col justify-between shadow-xl relative"
            >
              <Quote className="w-8 h-8 text-teal-500/20 absolute top-5 right-5 pointer-events-none" />
              ...
              <div>
                ...
                <p className="text-xs text-slate-300 leading-relaxed mb-5 sm:mb-6 italic">
                  &ldquo;{r.quote}&rdquo;
                </p>
              </div>

              <div className="pt-3.5 sm:pt-4 border-t border-slate-800 flex items-center justify-between">
                <div>
                ...
                <div className="text-[10px] text-slate-500 shrink-0">{r.date}</div>
              </div>
            </div>
          ))}
        </div>
