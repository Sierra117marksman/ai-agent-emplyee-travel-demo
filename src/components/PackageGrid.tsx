...
export default function PackageGrid({ onSelectPackageForArjun }: PackageGridProps) {
  ...
  return (
    <section id="packages" className="py-14 sm:py-20 bg-slate-900 border-b border-slate-800 text-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 sm:mb-12">
          ...
          {/* Category Filter Pills - horizontal swipeable on mobile */}
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 md:pb-0 -mx-4 px-4 sm:mx-0 sm:px-0 flex-nowrap sm:flex-wrap">
            {categories.map((cat) => (
              <button
                key={cat.id}
                type="button"
                onClick={() => setSelectedCategory(cat.id)}
                className={`text-xs font-semibold px-3.5 py-2 sm:px-4 sm:py-2 rounded-xl transition-all whitespace-nowrap active:scale-95 shrink-0 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filteredPackages.map((pkg) => (
            ...
            >
              ...
              <div className={`h-36 sm:h-40 bg-gradient-to-br ${pkg.gradient} p-4 sm:p-6 flex flex-col justify-between relative overflow-hidden`}>
                <div className="absolute inset-0 bg-black/20" />
                <div className="relative z-10 flex items-center justify-between">
                  <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider px-2.5 py-0.5 sm:py-1 rounded-full bg-black/40 text-white backdrop-blur-sm border border-white/10">
                    {pkg.category}
                  </span>
                  <div className="flex items-center gap-1 text-xs font-bold px-2.5 py-0.5 sm:py-1 rounded-full bg-black/40 text-amber-300 backdrop-blur-sm border border-white/10">
              ...
              <div className="p-4 sm:p-6 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-3 sm:mb-4 pb-3 sm:pb-4 border-b border-slate-850">
                ...
                <div className="pt-4 sm:pt-5 border-t border-slate-850 mt-3 sm:mt-4">
                  <div className="flex items-end justify-between mb-3 sm:mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs sm:text-sm font-bold text-teal-300">₹2,000 Only</div>
                    </div>
                  </div>
